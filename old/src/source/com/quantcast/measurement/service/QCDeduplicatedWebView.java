    ...
    implements QCNotificationListener
{
    ...
    void appendUserAgent(boolean flag)
    {
        ...
        if(!flag1 && flag)
            websettings.setUserAgentString((new StringBuilder()).append(s).append(" QuantcastSDK").append("/").append("1_1_0").append("/").append(QCMeasurement.INSTANCE.getApiKey()).toString());
        else
        if(flag1 && !flag)
        {
            int i = matcher.start();
