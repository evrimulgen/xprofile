...
public class Util
{
    ...
    */
    ...
    public void resolveFromClassFile(SootClass aClass, InputStream is, String filePath, Collection<Type> references)
    {
        ...
        // NOTE(tony): make sure we are at SIGNATURES. This is a hack before we
        // make xml reading demand-driven. We remember the current resolving
        // level, and recover it later.
        int oldLvl = bclass.resolvingLevel();
        if (oldLvl < SootClass.SIGNATURES) {
            bclass.setResolvingLevel(SootClass.SIGNATURES);
        }
        // END of tweak
                        ...
                        for(int k = 0; k < exceptions.number_of_exceptions; k++)
{
        ...
        // NOTE(tony): recover the old resolving level
        bclass.setResolvingLevel(oldLvl);
        // END of tweak
